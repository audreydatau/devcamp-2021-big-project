...
func (p *Module) GetProductBatch(limit int, offset int,name string,description string) ([]ProductResponse, error) {
	var resp []ProductResponse
	var err error

	resp, err = p.Storage.GetProductBatch(limit, offset, name, description)
	if err != nil {
		log.Println("[ProductModule][GetProduct] problem getting storage data, err: ", err.Error())
		return resp, err
	}
