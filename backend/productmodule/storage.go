...
func (s *storage) GetProductBatch(limit int, offset int,name string, description string) ([]ProductResponse, error) {
	resp := make([]ProductResponse, 0)

	rows, err := s.ProductDB.Query(getProductBatchQuery, limit, offset, name, description)
	if err != nil {
